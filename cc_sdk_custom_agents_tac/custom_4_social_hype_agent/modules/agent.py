...
Handles WebSocket connections, content processing, and Claude Code SDK integration.
"""
...
class SocialHypeAgent:
    ...
    async def _analyze_and_notify(self, text: str, matched_keywords: List[str]) -> Dict[str, str]:
        """
        Use Claude Code SDK with custom tools for content analysis.
        ...
        try:
            async with websockets.connect(uri) as websocket:
                ...
                config_table.add_row(
                    "Connection",
                    "[green]✅ Connected to Bluesky firehose[/green]"
                )

                # Add queue processor status
                config_table.add_row(
                    "Queue Processor",
                    "[green]🔄 Running with notifications[/green]"
                )
